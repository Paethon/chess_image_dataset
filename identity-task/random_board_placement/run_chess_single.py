...
blender = "blender "
basecommand = "-b /data/Dropbox/17_18_ws/iccv_workshop/SimilarityChessGenBoardRandomPositionBig/chessboard_single.blend -P /data/Dropbox/17_18_ws/iccv_workshop/SimilarityChessGenBoardRandomPositionBig/chess_single.py -- "
baseimgdir = "/data/chessboard_data/similarity/"

for diff in args.diff:
    # Check if dir exists, otherwise create
    checkdir = "{}/random_board_images_big_diff{}".format(baseimgdir, diff)
    if not os.path.isdir(checkdir):
        print("Generating directory {}".format(checkdir))
        os.makedirs(checkdir)
    # Generate images
    for start, stop in borders:
        parameters = "--diff {} --start {} --stop {}".format(
            diff, start, stop)
