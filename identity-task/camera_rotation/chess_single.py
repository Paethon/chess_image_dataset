...
def get_args():
    ...
    parser.add_argument('-c', '--basedir', type=str, help="Basedir")
...
d = args.diff
with open(args.basedir+"camerarot_diff" + str(d) + "/labels.txt", "w") as f:
    for i in range(args.start, args.stop):
        # Make mirrored image
        remove_pawn_copies()
        conf = create_random_config()
        apply_config(conf)
        apply_config(conf, xoffset=9, yoffset=0)
        filename = "single_chess_mirror_" + str(i).zfill(5) + ".jpg"
        # Select camera position
        if args.rotate:
            position_camera_on_sphere(*left_board_center, r=random.uniform(23, 24),
                                      thetaxy=random.uniform(0, 2 * pi),
                                      thetayz=random.uniform(pi / 6, pi / 3))
        render_to_file(
            args.basedir + "camerarot_diff" + str(d) + "/" + filename)
        f.write(filename + " 0\n")
        # Make non-mirrored config
        remove_pawn_copies()
        conf = create_random_config()
        apply_config(conf)
        apply_config(flip_pawns(conf, d), xoffset=9, yoffset=0)
