...
with open("/data/chessboard_data/similarity/labels.txt", "w") as f:
    for i in range(args.start, args.stop):
        ...
        y = random.uniform(-22, -3)
        delta = abs((y + 3) / 19)
        xleft = delta * 3 + 1.5
        xright = -delta * 3.5 - 2.5
        x = random.uniform(xleft, xright)
        position_camera(x, y, 25.66)
        render_to_file(
            "/data/chessboard_data/similarity/trans_images_diff" + str(d) + "/" + filename)
        f.write(filename + " 0\n")
